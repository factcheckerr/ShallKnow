...
from entity_linking_utils import ner_el_func, check_if_synonyms_in_sentence, get_triples, start_entity_linking, \
    get_processed_data, read_processed_claims, save_processed_triples_data
...
class Processor:
    ...
    def start_entity_linking_task(self,args, files_path, entities_dict, error_ent_dict, relations_dict):
        ...
        while True:
            ...
            if len(tpls) > 0:
                ...
                store_results(output_file, tpls, id, processed_sentences_file_path)
                # with open(output_file, 'a') as file:
                #     file.write(str(tpls) + '\n')
                # with open(processed_sentences_file_path, 'a') as file:
                #     file.write(f"Processed ID: {id}\n")
    ...
    def process_document_api(self, claim, args, entities_dict,error_ent_dict, relations_dict):
        ...
        if not claim1.startswith("folder:"):
            ...
            # for para in paragraphs:
                ...
                # }
                # self.process_document(args, tag_info, triple_claim_dict)
                # triples, entities_dict, relations_dict, error_ent_dict = start_entity_linking(triple_claim_dict, args,
                #                                                                               entities_dict,
                #                                                                               error_ent_dict,
                #                                                                               relations_dict,
                #                                                                               bulk_triples=False)
    ...
    def is_valid_triplet(self, tpl, sentence):
        ...
        for key, value in tpl.items():
            ...
            if not (value in sentence or check_if_synonyms_in_sentence(sentence, value)):
...
def argparse_default(description=None):
    ...
    # parser.add_argument("--dataset_path", default='/home/umair/PycharmProjects/TripleExtraction/data/NELA/')
